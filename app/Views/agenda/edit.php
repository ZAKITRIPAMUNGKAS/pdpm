...
<div class="info-badge">
    ...
    <strong>ID Agenda:</strong> #<?= $agenda['id'] ?> | 
    <strong>Dibuat:</strong> <?= date('d M Y H:i', strtotime($agenda['created_at'] ?? 'now')) ?>
</div>
...
<div class="card shadow-sm">
    <div class="card-body p-4">
        ...
        <form action="<?= site_url('admin-agenda/update/' . $agenda['id']) ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="form-section">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control form-control-lg" id="nama_kegiatan" 
                           name="nama_kegiatan" value="<?= old('nama_kegiatan', $agenda['nama_kegiatan']) ?>" 
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select form-select-lg" id="tingkat_agenda" name="tingkat_agenda" onchange="toggleCabangSelect()">
                            <option value="daerah" <?= old('tingkat_agenda', $agenda['tingkat_agenda'] ?? 'daerah') == 'daerah' ? 'selected' : '' ?>>
                                🏢 Agenda Daerah (Untuk Semua Cabang)
                            </option>
                            <option value="cabang" <?= old('tingkat_agenda', $agenda['tingkat_agenda'] ?? 'daerah') == 'cabang' ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-6 mb-3" id="cabang-select-container" 
                         style="display: <?= ($agenda['tingkat_agenda'] ?? 'daerah') == 'cabang' ? 'block' : 'none' ?>;">
                        ...
                        <select class="form-select form-select-lg" id="id_cabang_khusus" name="id_cabang_khusus">
                            ...
                            <?php foreach ($cabang_list as $cabang): ?>
                                <option value="<?= $cabang['id'] ?>" 
                                    <?= old('id_cabang_khusus', $agenda['id_cabang_khusus'] ?? '') == $cabang['id'] ? 'selected' : '' ?>>
                                    <?= esc($cabang['nama_cabang']) ?> 
                                    <?php if (isset($cabang['wilayah'])): ?> 
                                        - <?= esc($cabang['wilayah']) ?> 
                                    <?php endif; ?>
                                </option>
                            <?php endforeach; ?>
                ...
                <div class="alert alert-info alert-dismissible fade show" role="alert">
                    ...
                    <strong>Agenda Cabang:</strong> Sebagai admin cabang, agenda yang Anda edit akan tetap menjadi agenda khusus cabang Anda.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
                <?php endif; ?>
            ...
            <div class="form-section">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control form-control-lg" id="lokasi" 
                           name="lokasi" value="<?= old('lokasi', $agenda['lokasi']) ?>" 
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="latitude" name="latitude" 
                               value="<?= old('latitude', $agenda['latitude']) ?>" placeholder="-7.56789" required>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="longitude" name="longitude" 
                               value="<?= old('longitude', $agenda['longitude']) ?>" placeholder="110.12345" required>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <div class="input-group">
                            <input type="number" class="form-control" id="radius_meter" 
                                   name="radius_meter" value="<?= old('radius_meter', $agenda['radius_meter'] ?? 100) ?>" 
                ...
                <div class="alert alert-info" role="alert">
                    ...
                    Lat: <?= $agenda['latitude'] ?>, Long: <?= $agenda['longitude'] ?> 
                    (Radius: <?= $agenda['radius_meter'] ?? 100 ?> meter)
                </div>
            ...
            <div class="d-flex justify-content-between align-items-center mt-4">
                <div>
                    <small class="text-muted">
                        ...
                        Terakhir diperbarui: <?= date('d M Y H:i', strtotime($agenda['updated_at'] ?? $agenda['created_at'] ?? 'now')) ?>
                    </small>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <button type="submit" class="btn btn-lg btn-danger px-4">
                        <i class="bi bi-save me-2"></i>Perbarui Agenda
                    </button>
                </div>
            </div>
        </form>
...
function toggleCabangSelect() {
    ...
    if (tingkatSelect && tingkatSelect.value === 'cabang') {
        cabangContainer.style.display = 'block';
        cabangSelect.required = true;
    } else if (cabangContainer) {
...
document.addEventListener('DOMContentLoaded', function() {
    if (document.getElementById('tingkat_agenda')) {
        toggleCabangSelect();
    }
});
</script>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Initialize map on page load with existing values
        updateMap();
